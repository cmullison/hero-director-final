...
export function GoogleVideoGenerator() {
  const [prompt, setPrompt] = useState("");
  const [isGenerating, setIsGenerating] = useState(false);
  const [videoJobs, setVideoJobs] = useState<GoogleVideoJob[]>([]);
  ...
  const loadVideoJobs = async () => {
    try {
      const response = await fetch("/api/google-video-jobs", {
        credentials: "include",
      });
      ...
      console.error("Failed to load Google video jobs:", error);
    }
  };
  ...
  const updateJobStatuses = async () => {
    ...
    for (const job of processingJobs) {
      try {
        const response = await fetch(`/api/google-video-jobs/${job.id}`, {
          credentials: "include",
        });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch("/api/generate-video-google", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && data.job) {
          ...
          setTimeout(() => updateJobStatuses(), 2000);
        } else {
  ...
  const getStatusIcon = (status: GoogleVideoJob["status"]) => {
    switch (status) {
      case "pending":
      case "processing":
  ...
  const getStatusColor = (status: GoogleVideoJob["status"]) => {
    switch (status) {
      case "pending":
  ...
  return (
    <div className="space-y-4">
      ...
      {videoJobs.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-medium">Google Video Generation Jobs</h3>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            a.download = `google-video-${job.id}.mp4`;
                            a.click();
                          }}
                        >
        ...
        <div className="flex h-64 items-center justify-center rounded-md border border-dashed">
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              No Google video generation jobs yet
            </p>

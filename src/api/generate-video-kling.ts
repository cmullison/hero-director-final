...
interface GenerateVideoKlingParams {
  prompt: string;
  duration?: 5 | 10;
  cfg_scale?: number;
  start_image?: string;
...
function generateJobId(): string {
  return `kling_video_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
}
...
// Start a new Kling video generation job
export async function startKlingVideoGeneration(
  params: GenerateVideoKlingParams,
  env: any,
  request: Request
  ...
  // Log params but truncate start_image to avoid flooding logs
  const loggableParams = {
    ...params,
    ...(params.start_image && { start_image: `[BASE64_IMAGE_${params.start_image.length}_CHARS]` })
  };
  console.log("Starting Kling video generation with params:", loggableParams);
  ...
  try {
    ...
    const model = 'kwaivgi/kling-v2.0'; // Kling v2.0 model
    
    // Prepare Replicate API request with Kling parameters
    const replicateBody = {
      input: {
        prompt: params.prompt,
        ...(params.duration !== undefined && { duration: params.duration }),
        ...(params.cfg_scale !== undefined && { cfg_scale: params.cfg_scale }),
        ...(params.start_image !== undefined && { start_image: params.start_image }),
        ...(params.aspect_ratio !== undefined && { aspect_ratio: params.aspect_ratio }),
        ...(params.negative_prompt !== undefined && { negative_prompt: params.negative_prompt })
      }
    };

    // Log request but truncate start_image
    const loggableBody = {
      input: {
        ...replicateBody.input,
        ...(replicateBody.input.start_image && { 
          start_image: `[BASE64_IMAGE_${replicateBody.input.start_image.length}_CHARS]` 
        })
      }
    };
    console.log("Sending request to Replicate:", loggableBody);
    ...
    // Save job to database (don't store the actual start_image to avoid DB size issues)
    const settings = {
      duration: params.duration || 5,
      cfg_scale: params.cfg_scale || 0.5,
      aspect_ratio: params.aspect_ratio || "16:9",
      negative_prompt: params.negative_prompt || "",
      ...(params.start_image && { has_start_image: true })
    };
    ...
    const insertResult = await env.DB.prepare(`
      INSERT INTO video_jobs (
        id, userId, prompt, model, aspect_ratio, status, 
    ...
    console.error("Error starting Kling video generation:", error);
    return {
      success: false,
...
async function saveVideoToR2(
  ...
  try {
    ...
    const modelName = 'kling-v2'; // Use a cleaner name for Kling
    const key = `video-generations/${modelName}/${datePrefix}/${job.id}.mp4`;
    
    // Settings are now safe to use directly since we don't store start_image
    const settingsForMetadata = job.settings || '';
    ...
    if (error instanceof Error) {
      console.error('Error details:', {
        ...
        stack: error.stack,
        jobId: job.id,
        hasStartImage: job.settings?.includes('has_start_image') || false,
        settingsLength: job.settings?.length || 0,
      });
    }
...
export async function getKlingVideoJobStatus(
  jobId: string,
  env: any,
  request: Request,
...
// Get all Kling video jobs for a user
export async function getUserKlingVideoJobs(
  env: any,
  request: Request,
  limit: number = 50,
  offset: number = 0
  ...
  try {
    const result = await env.DB.prepare(`
      ...
      WHERE userId = ? AND model = 'kwaivgi/kling-v2.0'
    ...
    console.error("Error getting user Kling video jobs:", error);
    return {
      success: false,
...
// Cancel a Kling video generation job
export async function cancelKlingVideoJob(
  jobId: string,
  env: any,
  request: Request
): Promise<ActionResult> {
